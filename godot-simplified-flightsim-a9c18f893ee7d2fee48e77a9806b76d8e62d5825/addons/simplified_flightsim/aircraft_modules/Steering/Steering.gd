...
signal control_surface_update(axis, value)
...
func process_physic_frame(delta):
	...
	if axis_z != 0:
		...
		emit_signal("control_surface_update", "aileron", axis_z)
	...
	if axis_x != 0:
		...
		emit_signal("control_surface_update", "elevator", axis_x)
	...
	if axis_y != 0:
		...
		emit_signal("control_surface_update", "rudder", axis_y)
